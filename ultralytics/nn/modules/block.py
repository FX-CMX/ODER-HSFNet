...
from .dynamic_residual_attention_2D import RMSNorm2D, BlockAttnRes2D, DynamicFuseModule
from .transformer import TransformerBlock

_DEFORMEDGE_ENTROPY_CACHE = weakref.WeakKeyDictionary()
...
__all__ = (
    ...
    "DSC3k2_Deform",
    "_VSSBlockCompat",
    "Scheme1_DSConv_HyperACE",
    "Scheme3_DSConv_HyperACE",
    "HSSF",
    "PreDSConv_HyperACE",
    "VSS_HyperACE",
    "DSConvAttnVSSBlock",
...
class SpatialFullPAD_Tunnel(nn.Module):
    """
    Spatially-gated FullPAD tunnel.

    This module is an isolated replacement candidate for FullPAD_Tunnel. Instead of using one global scalar gate,
    it predicts a per-location gate map from the original and HyperACE-enhanced features:

        out = original + gamma * gate_map([original, enhanced]) * enhanced

    gamma is initialized to 0, so the module is exactly equivalent to an identity residual path at initialization.
    This keeps training stable and prevents the new experiment from affecting existing FullPAD_Tunnel users.
    """

    def __init__(self, c, reduction=8, gamma_init=0.0):
        super().__init__()
        c = int(c)
        hidden = max(8, c // int(max(1, reduction)))
        self.gate = nn.Sequential(
            nn.Conv2d(2 * c, hidden, 1, 1, 0, bias=False),
    ...
    def forward(self, x):
        original, enhanced = x
        gate_map = self.gate(torch.cat((original, enhanced), dim=1))
        return original + self.gamma * gate_map * enhanced


class DepthAwareFullPAD_Tunnel(nn.Module):
    """
    Depth-aware spatial FullPAD tunnel with a normalized vertical-coordinate prior.

    Compared with SpatialFullPAD_Tunnel, this variant concatenates a normalized y-coordinate map to the gate input:

        gate_map = Gate([original, enhanced, y_coord])
        out = original + gamma * gate_map * enhanced

    The y-coordinate is a weak classroom-depth prior. It lets the gate learn non-uniform feature injection across
    vertical depth regions, which matches HCCB's asymmetric occlusion and rear-row small-target degradation.
    gamma is initialized to 0, preserving the original feature path at initialization.
    """
    ...
    def __init__(self, c, reduction=8, gamma_init=0.0):
        ...
        hidden = max(8, c // int(max(1, reduction)))
        self.gate = nn.Sequential(
            nn.Conv2d(2 * c + 1, hidden, 1, 1, 0, bias=False),
            nn.BatchNorm2d(hidden),
            nn.SiLU(),
            nn.Conv2d(hidden, 1, 1, 1, 0, bias=True),
            nn.Sigmoid(),
    ...
    def _y_coord(x):
        ...
        return y.expand(b, 1, h, w)

    def forward(self, x):
        original, enhanced = x
        y_coord = self._y_coord(original)
        gate_map = self.gate(torch.cat((original, enhanced, y_coord), dim=1))
        return original + self.gamma * gate_map * enhanced


class _TinyStateMixer1D(nn.Module):
    """Lightweight 1D state mixer used only for topology weight generation."""

    def __init__(self, channels):
        super().__init__()
        self.in_proj = nn.Linear(channels, channels, bias=False)
        self.dw_conv = nn.Conv1d(channels, channels, 3, 1, 1, groups=channels, bias=False)
    ...
    def forward(self, x):
        y = self.in_proj(x)
        y = self.dw_conv(y.transpose(1, 2).contiguous()).transpose(1, 2).contiguous()
        y = self.norm(y)
        y = self.out_proj(self.act(y))
        return y


class ODER(nn.Module):
    ...
    def __init__(
        ...
        align_corners=True,
        eps=1e-6,
        gamma_scale_range=(0.85, 1.15),
        adapt_warmup_steps=None,
        *args,
        **kwargs
    ):
        super().__init__()
        self.k = int(max(1, k_samples))
        self.c2 = int(c2)
        self.max_offset = float(max_offset)
        self.temperature = float(max(1e-3, temperature))
        self.warmup_steps = int(max(0, warmup_steps))
        
        # Legacy ablation parser
        if isinstance(padding_mode, bool) or isinstance(gamma_scale_range, bool):
            self.use_deao = padding_mode
            self.use_hdsg = align_corners
            self.use_scale = gamma_scale_range
            # Reset to defaults
            padding_mode = 'border'
            align_corners = True
            eps = 1e-6
            gamma_scale_range = (0.85, 1.15)
            adapt_warmup_steps = warmup_steps
        else:
            self.use_deao = True
            self.use_hdsg = True
            self.use_scale = True
        ...
        if isinstance(c1, (list, tuple)):
            raise ValueError('ODER is single-input only. Use an external cross-scale fusion block before this operator.')

        self.in_proj = nn.Conv2d(int(c1), self.c2, 1, 1, 0, bias=False)

        cp = max(16, self.c2 // int(max(1, squeeze)))
        self.prior_reduce = nn.Conv2d(self.c2, cp, 1, 1, 0, bias=False)
        self.prior_bn = nn.BatchNorm2d(cp)
        self.prior_act = nn.SiLU()

        self.offset_head = nn.Conv2d(cp, 2 * self.k, 3, 1, 1, bias=True)
        nn.init.constant_(self.offset_head.weight, 0)
        nn.init.constant_(self.offset_head.bias, 0)

        self.state_mixer = _TinyStateMixer1D(cp)
        self.pos_proj = nn.Linear(2, cp, bias=False)
        self.pos_gain = nn.Parameter(torch.tensor(0.1))
        self.weight_head = nn.Linear(cp, self.k, bias=True)

        self.out_proj = nn.Conv2d(self.c2, self.c2, 1, 1, 0, bias=True)
        ...
        self.gamma_scale_head = nn.Conv2d(cp, 1, 1, 1, 0, bias=True)
        nn.init.constant_(self.gamma_scale_head.weight, 0)
        nn.init.constant_(self.gamma_scale_head.bias, 0)

        if isinstance(gamma_scale_range, bool):
            gamma_scale_range = (0.85, 1.15)
        g_lo, g_hi = gamma_scale_range
        self.gamma_scale_min = float(min(g_lo, g_hi))
        self.gamma_scale_max = float(max(g_lo, g_hi))
        if adapt_warmup_steps is None:
            adapt_warmup_steps = warmup_steps
        self.adapt_warmup_steps = int(max(0, adapt_warmup_steps))
    ...
    def entropy_penalty(self):
        entropy = _DEFORMEDGE_ENTROPY_CACHE.get(self)
        if entropy is None:
            return None
        return math.log(float(self.k)) - entropy

    @staticmethod
    def _map_to_range(v, lo, hi):
        return lo + (hi - lo) * torch.sigmoid(v)

    def _adapt_ratio(self, x_dtype):
        if not self.training:
            return torch.tensor(1, dtype=x_dtype, device=self.gamma.device)
        if self.adapt_warmup_steps <= 0:
            return torch.tensor(1, dtype=x_dtype, device=self.gamma.device)
        return torch.clamp(self._step.float() / float(self.adapt_warmup_steps), min=0, max=1).to(x_dtype)

    def forward(self, x):
        x = self.in_proj(x)
        b, c, h, w = x.shape

        prior = self.prior_act(self.prior_bn(self.prior_reduce(x)))
        if getattr(self, 'use_deao', True):
            raw_offsets = self.offset_head(prior)
        else:
            raw_offsets = torch.zeros((b, 2 * self.k, h, w), device=x.device, dtype=x.dtype)

        temp = torch.tensor(self.temperature, device=x.device, dtype=x.dtype).view(1, 1, 1)
        gate_ctx = F.adaptive_avg_pool2d(prior, 1)
        if getattr(self, 'use_scale', True):
            gamma_scale = self._map_to_range(
                self.gamma_scale_head(gate_ctx), self.gamma_scale_min, self.gamma_scale_max
            ).view(b, 1, 1, 1)
        else:
            gamma_scale = torch.ones((b, 1, 1, 1), device=x.device, dtype=x.dtype)

        if self.training:
            self._step += 1
        adapt_ratio = self._adapt_ratio(x.dtype)
        gamma_scale = 1 + adapt_ratio * (gamma_scale - 1)

        if not self.training:
            max_mag = torch.tensor(self.max_offset, device=x.device, dtype=x.dtype)
        else:
            if self.warmup_steps > 0:
                warm = torch.clamp(self._step.float() / float(self.warmup_steps), min=0, max=1).to(x.dtype)
                max_mag = self.max_offset * warm
            else:
                max_mag = torch.tensor(self.max_offset, device=x.device, dtype=x.dtype)
        max_mag = torch.clamp(max_mag, min=1e-3).reshape(1, 1, 1, 1)

        offsets = torch.tanh(raw_offsets) * max_mag

        prior_tokens = prior.flatten(2).transpose(1, 2).contiguous()
        base = self._base_grid(h, w, x.device, x.dtype).view(1, h * w, 2).expand(b, -1, -1)
        if getattr(self, 'use_hdsg', True):
            prior_tokens = prior_tokens + self.pos_gain * self.pos_proj(base)
            state = self.state_mixer(prior_tokens)
            if isinstance(self.weight_head, nn.Conv1d):
                edge_logits = self.weight_head(state.transpose(1, 2).contiguous()).transpose(1, 2).contiguous()
            else:
                edge_logits = self.weight_head(state)
            if edge_logits.dim() == 3 and edge_logits.shape[-1] != self.k and edge_logits.shape[1] == self.k:
                edge_logits = edge_logits.transpose(1, 2).contiguous()
            alpha = torch.softmax(edge_logits / temp, dim=-1)
            alpha = alpha.transpose(1, 2).reshape(b, self.k, h, w).contiguous()
            entropy = -(alpha * torch.log(alpha + self.eps)).sum(dim=1).mean()
            _DEFORMEDGE_ENTROPY_CACHE[self] = entropy
        else:
            alpha = torch.full((b, self.k, h, w), 1.0 / self.k, device=x.device, dtype=x.dtype)
            _DEFORMEDGE_ENTROPY_CACHE[self] = None

        off = offsets.view(b, self.k, 2, h, w).permute(0, 1, 3, 4, 2).contiguous()
        off_x = off[..., 0] * (2 / max(w - 1, 1))
        off_y = off[..., 1] * (2 / max(h - 1, 1))
        off_norm = torch.stack([off_x, off_y], dim=-1)

        base_grid = self._base_grid(h, w, x.device, x.dtype)
        grid = torch.clamp(base_grid.unsqueeze(1) + off_norm, -1, 1)

        x_rep = x.repeat_interleave(self.k, dim=0).contiguous()
        grid_rep = grid.view(b * self.k, h, w, 2).contiguous()
        ...
        sampled = sampled.view(b, self.k, c, h, w).contiguous()

        msg = (sampled * alpha.unsqueeze(2)).sum(dim=1)
        out = x + self.gamma * gamma_scale * self.out_proj(msg)
        return out


class DeformableDSConv(nn.Module):
    """Deformable depthwise-separable convolution with DCNv4 fallback."""

    def __init__(self, c_in, c_out, k=3, s=1, d=1, offset_groups=4):
        super().__init__()
        p = (d * (k - 1)) // 2
        self.k = k
        self.s = s
        ...
        try:
            ...
            self.dcn_channels = ((c_in + 15) // 16) * 16
            ans_group = 1
            for g in (4, 8, 2, 1):
                if self.dcn_channels % g == 0 and (self.dcn_channels // g) % 16 == 0:
                    ans_group = g
                    break
            self.group = ans_group
            self.pad_to = self.dcn_channels - c_in
            self.use_dcnv4 = (s == 1)
            if self.use_dcnv4:
                self.dcnv4 = DCNv4(
            ...
            print(f"Warning: DCNv4 load failed ({e}), fallback to standard conv!")
            self.use_dcnv4 = False

        self.dw = nn.Conv2d(c_in, c_in, k, s, p if p is not None else 1, d, groups=c_in, bias=False)
        self.pw = nn.Conv2d(self.dcn_channels if getattr(self, 'use_dcnv4', False) else c_in, c_out, 1, bias=False)
        self.bn = nn.BatchNorm2d(c_out)
        self.act = nn.SiLU()

    def forward(self, x):
        if getattr(self, 'use_dcnv4', False) and x.is_cuda:
            B, C, H, W = x.shape
            if self.pad_to > 0:
                x = torch.nn.functional.pad(x, (0, 0, 0, 0, 0, self.pad_to))
                C = self.dcn_channels
            x_reshaped = x.view(B, C, H * W).permute(0, 2, 1).contiguous()
            x_dcn = self.dcnv4(x_reshaped, shape=(H, W))
            x_out = x_dcn.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
            if self.pad_to > 0:
                x_out = x_out[:, : self.c_in, :, :]
            return self.act(self.bn(self.pw(x_out)))

        return self.act(self.bn(self.pw(self.dw(x))))


class DeformBottleneck(nn.Module):
    """Bottleneck variant used by DSC3k2_Deform."""
...
class DSC3k2_Deform(C2f):
    """Ablation variant that replaces DSC3k2 bottlenecks with DeformBottleneck."""
    ...
    def __init__(
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(DeformBottleneck(self.c, self.c, shortcut=shortcut, e=1, k1=k1, k2=k2) for _ in range(n))

class _VSSBlockCompat(nn.Module):
    """SS2D/Mamba-compatible lightweight block with safe local fallback."""
    ...
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):
        ...
        if mamba_module is not None:
            self.block = mamba_module(d_model=dim, d_state=d_state, d_conv=d_conv, expand=expand)
            self.use_mamba = True

    def forward(self, x):
        b, c, h, w = x.shape
        # Use Mamba only on CUDA. Some mamba_ssm kernels require CUDA even when the package is installed,
        # while YOLO builds strides with a CPU dummy forward during model construction.
...
class Scheme1_DSConv_HyperACE(nn.Module):
    """Compatibility implementation for legacy Scheme1 checkpoints."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.in_channels = [int(x) for x in c1] if isinstance(c1, (list, tuple)) else [int(c1)] * 3
        self.pre_shortcut = bool(pre_shortcut)
        self.dsconv = nn.ModuleList(DSConv(c, c, k=3, s=1, d=1) for c in self.in_channels)
        self.vss = nn.ModuleList(_VSSBlockCompat(c) for c in self.in_channels)
    ...
    def _run_branches(self, x_list, residual_mode=False):
        out = []
        for x, d, v in zip(x_list, self.dsconv, self.vss):
            y = d(x)
            y = v(y)
            if residual_mode and getattr(self, 'pre_shortcut', True):
                y = x + y
            out.append(y)
        return out

    def forward(self, x_list):
        # Supports both freshly-instantiated and legacy-unpickled objects.
        if not isinstance(x_list, (list, tuple)):
            x_list = [x_list]
        if hasattr(self, 'dsconv') and hasattr(self, 'vss'):
            out = self._run_branches(x_list, residual_mode=False)
        else:
            out = list(x_list)
        return self.hyperace(out) if hasattr(self, 'hyperace') else out


class Scheme3_DSConv_HyperACE(Scheme1_DSConv_HyperACE):
    """Compatibility implementation for legacy Scheme3 checkpoints."""

    def forward(self, x_list):
        if not isinstance(x_list, (list, tuple)):
            x_list = [x_list]
        if hasattr(self, 'dsconv') and hasattr(self, 'vss'):
            out = self._run_branches(x_list, residual_mode=True)
        else:
            out = list(x_list)
        return self.hyperace(out) if hasattr(self, 'hyperace') else out


class HSSF(Scheme1_DSConv_HyperACE):
    """
    Hypergraph-State Spatial Fusion.

    Paper-facing name for the fusion block used by ODER-HSFNet. The implementation first applies
    DSConv local structure enhancement and a VSS/Mamba-compatible state-space context branch to
    each input scale, then performs adaptive hypergraph multi-scale aggregation through HyperACE.
    """
    pass


class PreDSConv_HyperACE(Scheme1_DSConv_HyperACE):
    """Legacy compatibility alias kept for earlier experiment YAMLs/checkpoints."""
    pass


class VSS_HyperACE(Scheme1_DSConv_HyperACE):
    """Legacy compatibility alias for Ablation A: Mamba Only."""
    pass


class DeforAttnVSSBlock(nn.Module):
    """Deformable Attention VSS Block with SE Residual."""
    def __init__(self, c1, c2, d_state=16, d_conv=4, expand=2, deform_k=3, deform_groups=4, attn_reduction=0.25, shortcut=True):
        super().__init__()
        self.c1 = int(c1)
        self.c2 = int(c2)
        self.shortcut = bool(shortcut)
        self.in_proj = Conv(self.c1, self.c2, 1, 1)
        self.DWCconv = DeformableDSConv(self.c2, self.c2, k=int(deform_k), s=1, d=1, offset_groups=max(1, int(deform_groups)))
        self.ss2d = _VSSBlockCompat(self.c2, d_state=int(d_state), d_conv=int(d_conv), expand=int(expand))
        self.out_norm = nn.LayerNorm(self.c2)
        self.attn_res = _SEAttentionResidual(self.c2, reduction=float(attn_reduction))
        self.out_proj = Conv(self.c2, self.c2, 1, 1)
        self.short_proj = Conv(self.c1, self.c2, 1, 1, act=False) if self.c1 != self.c2 else nn.Identity()

    def forward(self, x):
        identity = self.short_proj(x)
        y = self.in_proj(x)
        y = self.DWCconv(y)
        y = self.ss2d(y)
...
class DSConvAttnVSSBlock(DeforAttnVSSBlock):
    """Legacy compatibility alias for DSConv+VSS block."""
    pass


class MS_DBAR_FuseModule(DynamicFuseModule):
    """Legacy compatibility alias for MS-DBAR fuse module."""
    pass

class VSS_DBAR_HyperACE(Scheme1_DSConv_HyperACE):
    """Legacy compatibility alias for VSS-DBAR variant."""
    pass


class MS_DBAR_HyperACE(Scheme1_DSConv_HyperACE):
    """Legacy compatibility alias for MS-DBAR variant."""
    pass


class DAFR_HyperACE(Scheme1_DSConv_HyperACE):
    """Legacy compatibility alias for DAFR variant."""
    pass


class _SEAttentionResidual(nn.Module):
    """Squeeze-and-Excitation Attention with Residual connection."""
    def __init__(self, c, reduction=0.25):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Linear(c, int(c * reduction), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(int(c * reduction), c, bias=False),
            nn.Sigmoid()
        )
