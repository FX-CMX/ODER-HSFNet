...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
        ...
        elif m in {Detect, DAOCDetect, QADetect, OCDetect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, DAOCDetect, QADetect, OCDetect, Segment, Pose, OBB}:
                m.legacy = legacy
            ...
            c2 = ch[f[-1]]
        elif m in {HSSF, Scheme1_DSConv_HyperACE, Scheme3_DSConv_HyperACE, PreDSConv_HyperACE, VSS_HyperACE, VSS_DBAR_HyperACE, MS_DBAR_HyperACE, DAFR_HyperACE}:
            legacy = False
            ...
            c2 = ch[f[0]]
        elif m in {SpatialFullPAD_Tunnel, DepthAwareFullPAD_Tunnel}:
            c2 = ch[f[0]]
            args = [c2, *args]
        else:
            c2 = ch[f]
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, DAOCDetect, QADetect, OCDetect, WorldDetect, v10Detect)):
                return "detect"
